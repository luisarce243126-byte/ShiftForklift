...
export default function App() {
  ...
  const handleAddRequest = (e) => {
    ...
    setVacations([
      { id: Date.now().toString(), employeeId, startDate: start, endDate: end, status: 'pending', notes: notes || 'Sin observaciones' },
      ...vacations
    ]);
  ...
  const exportToCSV = () => {
    ...
    const rows = vacations.map(v => {
      ...
      return [
        ...
        `"${v.startDate}"`,
        `"${v.endDate}"`,
        getDaysCount(v.startDate, v.endDate),
        `"${v.status}"`,
        `"${(v.notes || '').replace(/"/g, '""')}"`
      ].join(',');
    });
      ...
      <header className="bg-slate-800 border-b border-slate-700 sticky top-0 z-30 shadow-md">
        <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <button 
              onClick={exportToCSV} 
              className="flex items-center gap-2 px-3 py-2 text-sm bg-slate-700 hover:bg-slate-600 text-slate-200 rounded-lg"
            >
            ...
            <button 
              ...
              className="flex items-center gap-2 px-4 py-2 text-sm text-slate-950 font-semibold bg-amber-500 hover:bg-amber-400 rounded-lg"
            >
        ...
        <div className="flex border-b border-slate-700 mb-6 gap-8">
          {['calendar', 'requests', 'employees'].map(tab => (
            <button 
              ...
              className={`pb-4 px-1 flex items-center gap-2 font-medium text-sm border-b-2 capitalize ${
                activeTab === tab 
            ...
            >
              ...
              {tab === 'calendar' 
                ...
                : tab === 'requests' 
                  ? `Programaciones (${vacations.filter(v => v.status === 'pending').length})` 
                  : `Operadores (${employees.length})`}
            </button>
          ))}
        </div>
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 bg-slate-800 p-4 rounded-xl border border-slate-700 mb-6">
          <div className="flex gap-4">
            <select 
              ...
              className="bg-slate-900 text-slate-200 border border-slate-700 p-2 rounded-lg text-sm"
            >
        ...
        {activeTab === 'calendar' && (
          <div className="bg-slate-800 rounded-xl border border-slate-700 overflow-hidden">
            <div className="flex justify-between p-4 border-b border-slate-700 bg-slate-800/50">
              <h2 className="font-bold capitalize text-amber-400">
                {firstDay.toLocaleString('es-ES', { month: 'long', year: 'numeric' })}
              </h2>
              <div className="flex gap-2">
                ...
                <button 
                  ...
                  className="px-3 text-xs font-semibold bg-slate-700 hover:bg-slate-600 rounded text-slate-200"
                >
                  Hoy
                </button>
                <button 
            ...
            <div className="grid grid-cols-7 bg-slate-700 gap-px">
              {calendarDays.map((day, i) => !day ? (
                <div key={i} className="bg-slate-800 min-h-[90px] p-2" />
              ) : (
                <div key={day.dateKey} className="bg-slate-800 min-h-[90px] p-2">
                  ...
                  <div className="space-y-1 mt-1">
                    {filteredVacations
                      .filter(v => v.status !== 'rejected' && day.dateKey >= v.startDate && day.dateKey <= v.endDate)
                      .map(v => (
                        <div 
                          key={v.id} 
                          className={`p-1 rounded text-[11px] flex justify-between border ${
                            v.status === 'approved' 
                        ...
                        >
                          <span className="truncate">
                            {employees.find(e => e.id === v.employeeId)?.name.split(' ')[0] || 'N/A'}
                          </span>
                ...
                {filteredVacations.map(v => (
                  <tr key={v.id} className="hover:bg-slate-750">
                    <td className="p-4 font-semibold text-slate-200">
                      {employees.find(e => e.id === v.employeeId)?.name || 'Eliminado'}
                    </td>
                    ...
                    <td className="p-4 text-right">
                      {v.status === 'pending' && (
                        <>
                          <button 
                            onClick={() => setVacations(vacations.map(x => x.id === v.id ? { ...x, status: 'approved' } : x))} 
                            className="p-1 text-emerald-400 hover:text-emerald-300"
                          >
                            <Check className="w-4 h-4" />
                          </button>
                          <button 
                            onClick={() => setVacations(vacations.map(x => x.id === v.id ? { ...x, status: 'rejected' } : x))} 
                  ...
                  <div className="flex justify-between items-center mb-2">
                    ...
                    <div className="flex gap-1">
                      <button 
                        onClick={() => { 
                          setEditingEmp(emp); 
                          setEmpForm({ name: emp.name, dept: emp.department, pos: emp.position, maxDays: emp.maxDays }); 
          ...
          <div className="bg-slate-800 border border-slate-700 rounded-xl max-w-md w-full p-6 relative">
            ...
            <form onSubmit={handleAddRequest} className="space-y-3">
              <select 
                ...
                className="w-full bg-slate-900 border border-slate-700 text-slate-200 p-2 rounded text-sm" 
                required
              >
              ...
              <div className="grid grid-cols-2 gap-2">
                <input 
                  ...
                  className="bg-slate-900 border border-slate-700 text-slate-200 p-2 rounded text-sm" 
                  required 
                />
                <input 
                  type="date" 
              ...
              <textarea 
                placeholder="Notas / Turno de reemplazo" 
                value={newReq.notes} 
              ...
              <div className="flex justify-end gap-2">
                ...
                <button 
                  type="submit" 
                  className="px-4 py-2 bg-amber-500 text-slate-950 font-bold rounded text-sm hover:bg-amber-400"
                >
                  Guardar
                </button>
              </div>
            ...
            <form onSubmit={handleSaveEmployee} className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                <input 
                  ...
                  value={empForm.pos} 
                  onChange={e => setEmpForm({ ...empForm, pos: e.target.value })} 
                  className="bg-slate-900 border border-slate-700 text-slate-200 p-2 rounded text-sm" 
                  required 
                />
              </div>
              <input 
                type="number" 
                placeholder="Días Máximos Asignados" 
                value={empForm.maxDays} 
                onChange={e => setEmpForm({ ...empForm, maxDays: e.target.value })} 
                className="w-full bg-slate-900 border border-slate-700 text-slate-200 p-2 rounded text-sm" 
                required 
              />
              <div className="flex justify-end gap-2">
