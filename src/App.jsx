              ...
              <div className="flex gap-1.5 overflow-x-auto pb-1">
                {weekDays.map(day => {
                  ...
                  return (
                    ...
                    >
                      <span className="text-[10px] font-bold uppercase leading-tight">{day.dayName}</span>
                      <span className={`text-base font-extrabold leading-tight ${day.isWeekend && !isSelected ? 'text-red-400' : ''}`}>{day.dayNumber}</span>
                      {isToday && <span className="text-[8px] font-bold uppercase tracking-wider leading-tight">Hoy</span>}
                    </button>
                  );
                })}
              </div>
                ...
                {filteredOperators.map(op => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-full flex items-center gap-2.5 p-2 rounded-lg border text-left transition ${shift.color} ${
                        isLockedByAbsence && !isHistoricalWeek ? 'ring-2 ring-purple-400/60' : ''
                      } ${!editable && !isLockedByAbsence ? 'opacity-60' : 'active:scale-[0.98]'}`}
                    >
                      <div className={`shrink-0 w-8 h-8 rounded-lg flex items-center justify-center border ${shift.color}`}>
                        <IconComp className="w-4 h-4" />
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="font-bold text-[13px] truncate leading-tight">{op.name}</div>
                        <div className="text-[10px] opacity-80 truncate leading-tight">{op.id} · {op.zone}</div>
                      </div>
